...
/**
 * 보조스트림을 이용한 파일 복사
 * - FileInputStrem의 성능 향상을 위한 보조적인 스트림
 * - BufferedInputStrem: 하드디스크에서 읽어들이지 않고
 *   메모리 버퍼에 미리 읽어놓은 데이터를 읽어들임.
 *   - 속도가 아주 빠름.
 */
public class BufferedInputOutputExam01 { // class s
	...
	public static void main(String[] args) { // main s
		...
		try {
			...
			int bytesRead;
			while ((bytesRead = bis.read(buffer)) != -1) {	// -1 파일끝
				bos.write(buffer, 0, bytesRead);
			}
			bis.close();	// FileInputStream을 닫아준다.	원래는 finally에서 사용해야함
			bos.close();	// FileOutputStream을 닫아준다. 원래는 finally에서 사용해야함
