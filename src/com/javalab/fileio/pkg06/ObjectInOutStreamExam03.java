...
/**
 * 객체를 파일에 저장하는 예제 - 객체를 파일에 쓸때는 직렬화를 하고 - 파일에 써진 객체를 읽어올 떄는 역직렬화를 한다. - 이때 필요한
 * 것이 직렬화된 객체와 역직렬화된 객체가 같은 타입이냐가 중요하다. 그래서 serialVersionUID 필드가 필요하다.
 */
public class ObjectInOutStreamExam03 {
	...
	public static void main(String[] args) throws Exception {
		...
		// 객체를 (직렬화)해서 파일에 저장(객체의 필드를 바이트 형태로 변환해서 저장)
		...
		// FileInputStream에 ObjectInputStream 보조 스트림 연걸
		ObjectInputStream ois = new ObjectInputStream(fis);

		// 파일을 읽고 역직렬화해서 객체로 복원
		// byte형태로 파일에 저장해놓은 객체를 다시 불러들여서 자바 객체 형태로 복원
		Member member5 = (Member) ois.readObject(); // 객체가 4개라서 4번 읽
